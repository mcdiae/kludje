...
// see the caveats in the tested type to see if they apply to unit test failures
public class SerializedLambdasTest {
  ...
  public void testDoesNotImplementWriteReplace() {
    // init
    Foo foo = new Foo() {
      @Override
      public void foo() {}
    ...
    // invoke
    SerializedLambdas.toSerializedLambda(foo);
  }

  @Test(expected = SerializedLambdas.NotASerializedLambdaException.class)
  public void testNotASerializedLambda() {
    class NotALambda implements Foo {

      @Override
      public void foo() {}

      Object writeReplace() {
        return "I'm a string, not a SerializedLambda";
      }
    }
    // invoke
    SerializedLambdas.toSerializedLambda(new NotALambda());
  }

  @Test(expected = SerializedLambdas.InspectionException.class)
  public void testBadBehaviour() {
    class NotALambda implements Foo {
      ...
      Object writeReplace() {
        throw new RuntimeException("Misbehaving type");
      }
    }
    // invoke
    SerializedLambdas.toSerializedLambda(new NotALambda());
  ...
  public void testOnNull() {
    SerializedLambdas.toSerializedLambda(null);
  }
